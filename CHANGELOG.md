...
### 变更
- 版本号提升至 `2.6.5`，同步 GitHub / Gitee / npm。
...
### 修复
- **对话框「确定」按钮状态未实时更新**：统一新增 `setOkDisabled(shell, disabled)` 辅助函数，集中管理 link / image / video / audio / iframe 等对话框确定按钮的启用/禁用状态。
- 修复输入框内容变化后确定按钮未同步启用的问题（如链接地址、图片 URL、iframe 地址等）。

### 新增
- **浏览器自动化测试**：新增 Playwright 测试脚本：
  - `test-browser-features.mjs`：覆盖 27 项基础功能（清空/输入/格式/列表/撤销重做/插入/链接/iframe/锚点/查找替换/源码/预览/导出/全屏等）。
  - `test-browser-extended.mjs`：覆盖 41 项扩展功能（复制/剪切/粘贴/字体/字号/段落格式/颜色/上下标/引用/代码块/缩进/行距/对齐/段间距/文字方向/大小写/字符边框/格式刷/表格/特殊字符/Emoji/图片/远程图片/视频/音频/文件/音乐/图表/涂鸦/模板/自动排版/背景色/代码语言/全选/打印等）。
- 测试截图保存到 `test-screenshots/` 目录，包含源码视图、预览、全屏、最终状态等。

### 测试
- 基础浏览器测试：27/27 通过。
- 扩展浏览器测试：41/41 通过。
- 单元测试：`pnpm test` 42/42 通过；`pnpm check` 0 错误；`pnpm lint` 0 错误；`pnpm build:lib` 成功（sEditor.js 523.72 kB / sEditor.esm.js 761.11 kB）。
